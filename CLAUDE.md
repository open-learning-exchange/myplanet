...
- **Never scheduled** — the *Run workflow* button (linked from the release warning and the automerge stop), `gh workflow run playstore.yml`, or a `repository_dispatch` with `event_type: playstore`. `wait_minutes` waits on a runner for the next slot; `resume_automerge: true` dispatches the drain once nothing is owed to the track — published here, or already current, since either way the release that stopped the drain no longer does
