...
|---|---|---|---|---|---|
| **CodeRabbit** (`coderabbitai[bot]`) | Reviewer | auto on push + `@coderabbitai review` / `full review` / `resolve` / `approve` / `fix ci` / `fix ci commit` / `resolve merge conflict` / `pause`·`resume` / `regenerate summary` / `help` | minutes | only opt-in (`fix ci commit`) | auto-review skips drafts and pauses after 5 reviewed commits (config); per-dev review allowance; observed: review fails if head moves mid-review; `approve` needs `reviews.request_changes_workflow: true`; stores teachable per-repo learnings; will fact-check its own config if asked |
