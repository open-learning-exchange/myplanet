...
- Google Play takes about 48 saves per app per rolling 24h; at one release per master push (~6 min apart during an automerge drain) that ceiling is hit in an afternoon, leaving the newest build tagged and on GitHub but not on the internal track — which stops the drain, since it refuses to merge on top of an unpublished release
