...
- A red workflow on the base is re-run before the drain gives up (`BASE_RERUN_ATTEMPTS`, default 1): every base commit is a PR head that build + test passed on just before the squash merge, so a failure there is treated as flaky until it reproduces. A red workflow on a prepared commit gets the same one re-run (`HEAD_RERUN_ATTEMPTS`, default 1) before the PR is labelled `red` and dropped from the queue
- The dispatch menu is two fields — `dry_run` (defaults to `false`, so *Run workflow* drains for real and a dry run is the deliberate tick) and `max_merges`. Every other setting lives in the workflow's `env:` block, because three drains' logs show all of them were dispatched at their default every time; change one there, in a reviewable diff, and `playstore.yml`'s handover inherits it. That resume still passes `dry_run=false` explicitly rather than leaning on a default it cannot see
