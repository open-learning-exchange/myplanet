...
- A PR whose prepared commit goes red does not stop it either: build or test failing on that commit is a verdict on that PR alone, so it loses `automerge`, gains `red` (`RED_LABEL`), and the queue moves on — fix the failure and re-add `automerge`. What still stops the drain is the *absence* of a verdict on the prepared commit (no run ever appeared, or the wait timed out), since that says nothing about the PR and everything about the setup
