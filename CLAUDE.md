...
- Menu of three one-line fields (`resume_automerge`, `wait_minutes`, `dry_run`); `PLAYSTORE_TRACK`, `PLAYSTORE_DAILY_LIMIT` and `RETRY_MINUTES` are constants in the job's `env:`
- **Never scheduled** — the *Run workflow* button (linked from the release warning and the automerge stop), `gh workflow run playstore.yml`, or a `repository_dispatch` with `event_type: playstore`. `wait_minutes` waits on a runner for the next slot; `resume_automerge` (default **on** — unsticking the drain is why this gets pressed) dispatches the drain once nothing is owed to the track — published here, or already current, since either way the release that stopped the drain no longer does
- If the internal track is behind the newest GitHub release, re-uploads that release's signed `myPlanet-lite.aab` — the bundle the release workflow already built, so no rebuild and no new version code. It asks the track what it carries rather than trusting the newest `release.yml` run's warning — a track read opens an edit and deletes it unsaved, so it spends no save quota, and a release run that died before uploading looks exactly like one that published. The run is consulted for two things only: whether one is still in flight (wait for it) and, when the track cannot be read at all, whether it warned (upload blind) or not (leave the quota alone). Logic in `.github/scripts/playstore.sh`; track reads never commit their edit, so they spend no save quota
...
- Order is priority tier then PR number: PRs also labelled `priority` (`PRIORITY_LABEL`, blank = no tier) drain first, lowest number first within each tier
- For each labelled PR: merges the base branch in, bumps the version, waits for build + test to pass, then squash-merges
- A conflicting PR does not stop the drain (either detection: `mergeable: CONFLICTING` or the real `git merge`): it loses `automerge`, gains `conflict` (`CONFLICT_LABEL`, blank = only drop `automerge`), and the queue moves on — re-add `automerge` once resolved.
- A PR whose prepared commit goes red does not stop it either: build or test failing on that commit is a verdict on that PR alone, so it loses `automerge`, gains `failing` (`FAILING_LABEL`, dark red), and the queue moves on — fix the failure and re-add `automerge`. What still stops the drain is the *absence* of a verdict on the prepared commit (no run ever appeared, or the wait timed out), since that says nothing about the PR and everything about the setup
...
- A red workflow on the base is re-run before the drain gives up (`retries`, default 1): every base commit is a PR head that build + test passed on just before the squash merge, so a failure there is treated as flaky until it reproduces. A red workflow on a prepared commit gets the same one re-run (the same `retries`) before the PR is labelled `failing` and dropped from the queue
- The dispatch menu is four one-line fields: `dry_run` (defaults to `false`, so *Run workflow* drains for real and a dry run is the deliberate tick), `max_merges`, `labels` (all four labels in one comma-separated field, in the order `queue,priority,conflict,failing` — an empty slot means that label goes unused, a slot left off the end keeps the script's default) and `retries` (a 0–3 dropdown feeding both `BASE_RERUN_ATTEMPTS` and `HEAD_RERUN_ATTEMPTS`). Every other setting lives in the workflow's `env:` block, because three drains' logs show all of them were dispatched at their default every time; change one there, in a reviewable diff, and `playstore.yml`'s handover inherits it. That resume still passes `dry_run=false` explicitly rather than leaning on a default it cannot see
