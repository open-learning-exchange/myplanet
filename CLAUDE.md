...
- `.github/scripts/playstore-quota.sh` estimates when the next slot opens from the GitHub release history — 24h after the 48th-newest release, plus how many more free within the hour after that — and prints it in eastern time. It stays an estimate: a refused upload still counts there though it consumed no slot, and a re-run spends a slot without adding a release. `automerge.sh` reports the same line when a stopped publish stops the drain
