...
- Publishes to Google Play Store (internal track) with fallback retry; a refused upload (usually `Daily save quota exceeded.`) only warns — `playstore.yml` publishes it later
...
- Runs on a `*/30 * * * *` schedule (and `workflow_dispatch`); cron only ever runs the copy on the default branch
- Google Play accepts **48 saves per app per day** and answers the 49th with `Daily save quota exceeded.` — at one release per master push (~6 min apart during an automerge drain) that ceiling is hit in an afternoon, leaving the newest build tagged and on GitHub but not on the internal track, which stops the automerge drain
...
- `.github/scripts/playstore-quota.sh` estimates when the next save slot opens from the GitHub release history — the earlier of midnight Pacific (where Google API daily quotas roll over) and the 48th-newest release ageing out of a rolling 24h window — and prints it in eastern time. `automerge.sh` reports the same estimate when a stopped publish stops the drain
...
- A release that never reached the Play Store stops the drain; the stop now reports the estimated next save slot (eastern time) and `playstore.yml` retries the upload every 30 minutes
