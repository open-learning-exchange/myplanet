...
- A PR whose prepared commit goes red does not stop it either: build or test failing on that commit is a verdict on that PR alone, so it loses `automerge`, gains `red` (`red_label` input), and the queue moves on — fix the failure and re-add `automerge`. What still stops the drain is the *absence* of a verdict on the prepared commit (no run ever appeared, or the wait timed out), since that says nothing about the PR and everything about the setup
...
- Every input is dispatched at its default; `dry_run` defaults to `false`, so the *Run workflow* button drains for real and a dry run is the deliberate tick. `playstore.yml`'s resume passes `dry_run=false` explicitly rather than leaning on that
