...
| `docs/AGENT_SPELLBOOK.md` | You're summoning another AI agent (`@claude` `@coderabbit` `@codex` `@copilot` `@dependabot` `@devin` `@jules` `@openhands`) on a PR — who answers, how fast, and with what side effects. Its "The Armory" section covers maintaining the shared agent skills (`merge-prepping`, `kotlin-importing`) — how one skill repo feeds Claude Code, OpenHands, and Copilot |
