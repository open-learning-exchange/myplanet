# Refactoring Tasks - Repository Boundaries
...
### Add KDoc documentation to queryListFlow method in RealmRepository
The queryListFlow in RealmRepository has complex manual Realm lifecycle management. This should be documented to help future maintainers understand the Flow lifecycle.
...
### Verify VoicesLabelManager uses only repository callbacks
VoicesRepository already has addLabel/removeLabel but VoicesLabelManager still directly mutates Realm objects. Verify proper delegation.
...
### Document bulkInsert responsibilities in TransactionSyncManager
TransactionSyncManager has direct Realm access for bulk inserts. Document which methods belong where and add TODOs for future migration.
...
### Audit FileUtils usage for potential repository inclusion
FileUtils operations for SD card path handling are mixed with repository logic. Verify ResourcesRepository covers all resource file operations properly.
