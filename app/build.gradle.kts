...
android {
    ...
    val getProperty = { key: String ->
        project.findProperty(key) as? String ?: localProperties.getProperty(key) ?: gradleProperties.getProperty(key) ?: ""
    }
    ...
    buildTypes.forEach { buildType ->
        planets.forEach { planet ->
            val url = getProperty(planet.urlKey)
            val pin = getProperty(planet.pinKey)
...
tasks.register("generateVersionsXml") {
    val versionNameProvider = provider { android.defaultConfig.versionName }
    val versionsXmlFile = file("src/main/res/values/versions.xml")

    inputs.property("versionName", versionNameProvider)
    outputs.file(versionsXmlFile)

    doLast {
        versionsXmlFile.writeText(
            """
...
dependencies {
    ...
    implementation("com.mikepenz:materialdrawer:6.1.1@aar") { isTransitive = true }
