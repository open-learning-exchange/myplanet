...
import org.ole.planet.myplanet.ui.mylife.MyLifeAdapter
import org.ole.planet.myplanet.ui.mylife.MyLifeViewModel
...
open class BaseDashboardFragment : BaseDashboardFragmentPlugin(), NotificationCallback,
    ...
    private val myLifeViewModel: MyLifeViewModel by viewModels()
    ...
    fun onLoaded(v: View) {
        ...
        if (isRealmInitialized()) {
            myLifeAdapter = MyLifeAdapter { myLife ->
                myLife.title?.let { handleClickMyLife(it) }
            }
            v.findViewById<RecyclerView>(R.id.recyclerViewMyLife).apply {
                adapter = myLifeAdapter
    ...
    private fun observeUiState() {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            myLifeViewModel.myLifeData.collect {
                myLifeAdapter.submitList(it)
            }
        }
    }
