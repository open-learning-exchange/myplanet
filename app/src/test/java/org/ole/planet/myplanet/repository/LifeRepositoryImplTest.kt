...
class LifeRepositoryImplTest {
    ...
    fun setUp() {
        ...
        // spyk to allow mocking of RealmRepository methods like queryList and executeTransaction
        repository = spyk(LifeRepositoryImpl(databaseService), recordPrivateCalls = true)
    }
    ...
    fun getMyLifeByUserId_returnsSortedItems() = runTest {
        ...
        val unsortedList = listOf(item1, item2)

        coEvery {
            repository["queryList"](RealmMyLife::class.java, true, any<Function1<RealmQuery<RealmMyLife>, Unit>>())
        } returns unsortedList
    ...
    fun updateVisibility_callsExecuteTransactionAndTogglesIsVisible() = runTest {
        ...
        coEvery { repository["executeTransaction"](capture(transactionSlot)) } answers {
        ...
        coVerify(exactly = 1) { repository["executeTransaction"](any<Function1<Realm, Unit>>()) }
    }
    ...
    fun updateMyLifeListOrder_emptyList_doesNothing() = runTest {
        ...
        // Ensure no interactions with realm occur inside the transaction
    ...
    fun seedMyLifeIfEmpty_insertsItemsWhenNoExistingData() = runTest {
        ...
        assertTrue(insertedItems[0]._id != null)
        ...
        assertTrue(insertedItems[1]._id != null)
    }
