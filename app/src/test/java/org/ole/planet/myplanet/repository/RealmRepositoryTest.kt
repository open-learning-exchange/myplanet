...
class RealmRepositoryTest {
    ...
    fun `queryListFlow emits initial results then listener updates`() = runTest {
        ...
        // Setup the distinct check to fail by simulating diff pk or catching error so it emits
        every { frozenInitial.size } returns 1
        every { frozenUpdated.size } returns 2
    ...
    @Test
    fun `queryListFlow drops duplicate emission when changeSet is null and content is unchanged`() = runTest {
        val realmQuery = mockk<RealmQuery<TestRealmObject>>(relaxed = true)
        val initialResults = mockk<RealmResults<TestRealmObject>>(relaxed = true)
        val frozenInitial = mockk<RealmResults<TestRealmObject>>(relaxed = true)
        val frozenRealmInitial = mockk<Realm>(relaxed = true)
        ...
        every { frozenInitial.realm } returns frozenRealmInitial
        every { frozenInitial.isEmpty() } returns true // empty list bypasses schema primary key check, distincts true immediately if both empty
        every { frozenInitial.size } returns 0
        ...
        // Trigger listener with identical list and null changeset
        // distinctUntilChanged should drop this emission because it's identical (empty vs empty)
        listenerSlot.captured.onChange(initialResults, null)

        advanceUntilIdle()

        // Expecting only 1 emission still
        assertEquals(1, emittedLists.size)
        job.cancel()
    }
