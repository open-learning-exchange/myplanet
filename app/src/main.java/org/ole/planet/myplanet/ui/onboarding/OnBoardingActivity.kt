...
import org.ole.planet.myplanet.utilities.MapTileUtils
...
class OnBoardingActivity : AppCompatActivity() {
        ...
        if (requestCode == STORAGE_PERMISSION_CODE) {
            if (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                AlertDialog.Builder(this)
                    ...
                    .setMessage("Storage permission is required to save map data for offline use. You can grant the permission in the app settings.")
    ...
    private fun copyMapTileAssets() {
        val progressDialog = ProgressDialog(this).apply {
            setTitle("Preparing Map Data")
            setCancelable(false)
        ...
        lifecycleScope.launch {
            val result = withContext(Dispatchers.IO) {
                MapTileUtils.copyAssets(applicationContext) { fileName, bytesCopied, totalBytes ->
                    val progress = if (totalBytes > 0) ((bytesCopied * 100) / totalBytes).toInt() else 0
                    val message = "Copying $fileName...\n${bytesCopied / 1024} KB / ${totalBytes / 1024} KB"
            ...
            if (!success) {
                AlertDialog.Builder(this@OnBoardingActivity)
                    .setTitle("Map Data Error")
                    .setMessage("Failed to copy map tiles: $errorMessage. The map may not be available offline.")
                    .setPositiveButton("OK", null)
                    .show()
            }
        }
