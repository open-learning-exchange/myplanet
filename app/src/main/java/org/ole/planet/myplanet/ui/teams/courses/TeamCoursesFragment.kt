    ...
    private fun observeTeam() {
        lifecycleScope.launch {
            teamFlow.collect { team ->
                val coursesIds = team?.courses?.toList() ?: emptyList()
                val courses = withContext(Dispatchers.IO) {
                    coursesRepository.getByIds(coursesIds)
                }
                val teamCreator = team?.userId ?: ""
                setupAdapter(courses, teamCreator)
            }
        }
    }

    private fun setupAdapter(courses: List<RealmMyCourse>, teamCreator: String) {
        adapterTeamCourse = settings?.let { TeamCoursesAdapter(requireActivity(), courses.toMutableList(), teamCreator, it) }
