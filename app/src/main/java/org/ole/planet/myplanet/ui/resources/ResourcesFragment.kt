...
class ResourcesFragment : BaseRecyclerFragment<RealmMyLibrary?>(), OnLibraryItemSelected,
    ...
    private fun refreshResourcesData(onComplete: (() -> Unit)? = null) {
        if (!isAdded || requireActivity().isFinishing) return

        try {
            map = getRatings(mRealm, "resource", model?.id)
            val libraryList = getList(RealmMyLibrary::class.java).filterIsInstance<RealmMyLibrary>()
            ...
            val filteredLibraryList =
                if (currentSearchTags.isEmpty() && searchQuery.isEmpty()) {
                    applyFilter(libraryList).map { it }
                } else {
                    applyFilter(filterLibraryByTag(searchQuery, currentSearchTags)).map { it }
                }

            val sortedList = when (currentSort) {
                SortType.DATE -> sortLibraryList(filteredLibraryList)
                SortType.TITLE -> sortLibraryListByTitle(filteredLibraryList)
            }
    ...
    private fun clearTagsButton() {
        clearTags.setOnClickListener {
            ...
            subjects.clear()
            languages.clear()
            adapterLibrary.submitList(applyFilter(filterLibraryByTag("", searchTags))) {
                recyclerView.scrollToPosition(0)
            }
            showNoData(tvMessage, adapterLibrary.itemCount, "resources")
        }
    ...
    override fun onTagClicked(realmTag: RealmTag) {
        ...
        adapterLibrary.submitList(applyFilter(filterLibraryByTag(etSearch.text.toString(), searchTags))) {
            recyclerView.scrollToPosition(0)
        }
    ...
    override fun onTagSelected(tag: RealmTag) {
        ...
        adapterLibrary.submitList(applyFilter(filterLibraryByTag(etSearch.text.toString(), li))) {
            recyclerView.scrollToPosition(0)
        }
        showNoData(tvMessage, adapterLibrary.itemCount, "resources")
    }
    ...
    override fun filter(subjects: MutableSet<String>, languages: MutableSet<String>, mediums: MutableSet<String>, levels: MutableSet<String>) {
        ...
        this.levels = levels
        adapterLibrary.submitList(applyFilter(filterLibraryByTag(etSearch.text.toString().trim { it <= ' ' }, searchTags))) {
            recyclerView.scrollToPosition(0)
        }
        showNoData(tvMessage, adapterLibrary.itemCount, "resources")
    }
    ...
    override fun getData(): Map<String, Set<String>> {
        val libraryList = adapterLibrary.currentList
    ...
    private fun sortLibraryListByTitle(libraryList: List<RealmMyLibrary>): List<RealmMyLibrary> {
        return if (isTitleAscending) {
            libraryList.sortedBy { it.title?.lowercase(java.util.Locale.ROOT) }
        } else {
            libraryList.sortedByDescending { it.title?.lowercase(java.util.Locale.ROOT) }
        }
    }

    private fun sortLibraryList(libraryList: List<RealmMyLibrary>): List<RealmMyLibrary> {
