...
class AdapterProgressGrid(private val context: Context, private val list: List<StepProgress>) :
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder is ViewHolderMyProgress) {
            ...
            if (item.status?.isNotEmpty() == true) {
                holder.tvProgress.text = context.getString(R.string.percentage, String.format(Locale.US, "%.2f", item.percentage))
                if (item.completed) {
