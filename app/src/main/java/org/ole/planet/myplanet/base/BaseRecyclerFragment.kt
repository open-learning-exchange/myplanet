...
abstract class BaseRecyclerFragment<LI> : BaseRecyclerParentFragment<Any?>(), OnRatingChangeListener {
    ...
    suspend fun filterLibraryByTag(s: String, tags: List<RealmTag>): List<RealmMyLibrary> {
        ...
        if (tags.isNotEmpty()) {
            ...
            val tagsMap = tagsRepository.getTagsForResources(ids)
            list = list.filter { lib ->
                val libTags = tagsMap[lib.id]
                libTags != null && tags.any { tag -> libTags.any { it.tagId == tag.id } }
            }
        }
        return list
    }
    ...
    suspend fun filterCourseByTag(s: String, tags: List<RealmTag>): List<RealmMyCourse> {
        ...
        if (tags.isNotEmpty()) {
            val ids = list.mapNotNull { it.courseId }
            val tagsMap = tagsRepository.getTagsForCourses(ids)
            list = list.filter { course ->
                val courseTags = tagsMap[course.courseId]
                courseTags != null && tags.any { tag -> courseTags.any { it.tagId == tag.id } }
            }
        }
