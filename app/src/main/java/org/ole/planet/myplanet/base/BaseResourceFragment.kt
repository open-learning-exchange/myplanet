...
abstract class BaseResourceFragment : Fragment() {
    ...
    fun removeFromShelf(item: Any) {
        val userId = model?.id ?: return
        lifecycleScope.launch {
            if (item is RealmMyLibrary) {
                item.resourceId?.let {
                    resourcesRepository.updateUserLibrary(it, userId, false)
                    Utilities.toast(activity, getString(R.string.removed_from_mylibrary))
                }
            } else if (item is RealmMyCourse) {
                item.courseId?.let {
                    coursesRepository.leaveCourse(it, userId)
