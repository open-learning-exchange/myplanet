...
public class TakeExamFragment extends BaseExamFragment implements View.OnClickListener, CompoundButton.OnCheckedChangeListener, CameraUtils.ImageCaptureCallback {
    ...
    private void createSubmission() {
        ...
        if (TextUtils.isEmpty(id)) {
            sub.parentId = (!TextUtils.isEmpty(exam.getCourseId())) ? exam.getId() + "@" + exam.getCourseId() : exam.getId();
        } else {
            sub.parentId = (!TextUtils.isEmpty(exam.getCourseId())) ? id + "@" + exam.getCourseId() : id;
        }
        sub.userId = user.getId();
        sub.status = "pending";
        sub.type = type;
    ...
    public void startExam(RealmExamQuestion question) {
        ...
        if (sub.answers.size() > currentIndex) {
            ans = sub.answers.get(currentIndex).getValue();
        }
