...
import org.ole.planet.myplanet.utils.JsonUtils.getLong
...
private suspend fun getExamsByIds(examIds: List<String>): List<RealmStepExam> {
    ...
    override fun bulkInsertFromSync(realm: io.realm.Realm, jsonArray: com.google.gson.JsonArray) {
        val docs = ArrayList<JsonObject>(jsonArray.size())
        for (j in jsonArray) {
        ...
        val ids = docs.map { getString("_id", it) }.toTypedArray()
        val existingMap = realm.where(RealmSubmission::class.java)
            .`in`("_id", ids)
            .findAll()
            .associateBy { it._id }

        docs.forEach { insertSubmissionInternal(realm, it, existingMap) }
    }

    override fun insertSubmission(mRealm: io.realm.Realm, submission: JsonObject) {
        insertSubmissionInternal(mRealm, submission, emptyMap())
    }

    private fun insertSubmissionInternal(
        mRealm: io.realm.Realm,
        submission: JsonObject,
        existingMap: Map<String?, RealmSubmission>
    ) {
        if (submission.has("_attachments")) return

        val id = getString("_id", submission)
        ...
        try {
            ...
            var sub = existingMap[id]
                ?: mRealm.where(RealmSubmission::class.java).equalTo("_id", id).findFirst()
            val isNewSubmission = sub == null
            val hadLocalChanges = !isNewSubmission && sub.isUpdated
            val serverStatus = getString("status", submission)
    ...
    private fun updateBasicFields(
        ...
        sub?._rev = getString("_rev", submission)
        sub?.grade = getLong("grade", submission)
        sub?.type = getString("type", submission)
        sub?.uploaded = getString("_rev", submission).isNotEmpty()
        sub?.startTime = getLong("startTime", submission)
        sub?.lastUpdateTime = getLong("lastUpdateTime", submission)
        sub?.parentId = getString("parentId", submission)
        sub?.sender = getString("sender", submission)
        sub?.source = getString("source", submission)
        sub?.parentCode = getString("parentCode", submission)
        sub?.parent = gson.toJson(getJsonObject("parent", submission))
        sub?.user = gson.toJson(getJsonObject("user", submission))
    }
    ...
    private fun updateTeam(mRealm: io.realm.Realm, sub: RealmSubmission?, submission: JsonObject) {
        if (submission.has("team") && submission.get("team").isJsonObject) {
            ...
            teamRef._id = getString("_id", teamJson)
            teamRef.name = getString("name", teamJson)
            teamRef.type = getString("type", teamJson)
            sub?.teamObject = teamRef
        }
    }

    private fun updateMembership(mRealm: io.realm.Realm, sub: RealmSubmission?, submission: JsonObject) {
        val userJson = getJsonObject("user", submission)
        if (userJson.has("membershipDoc")) {
            val membershipJson = getJsonObject("membershipDoc", userJson)
            if (membershipJson.entrySet().isNotEmpty()) {
                val membership = mRealm.createObject(RealmMembershipDoc::class.java)
                membership.teamId = getString("teamId", membershipJson)
    ...
    private fun updateUserId(sub: RealmSubmission?, submission: JsonObject) {
        val userId = getString("_id", getJsonObject("user", submission))
    ...
    private fun updateAnswers(
        ...
        if (!skipOverwrite && submission.has("answers")) {
            ...
            for (i in 0 until answersArray.size()) {
                ...
                realmAnswer.value = getString("value", answerJson)
                realmAnswer.mistakes = getInt("mistakes", answerJson)
                realmAnswer.isPassed = getBoolean("passed", answerJson)
                ...
                realmAnswer.questionId = if (answerJson.has("questionId")) {
                    getString("questionId", answerJson)
                } else {
                    "$examIdPart-$i"
                }
