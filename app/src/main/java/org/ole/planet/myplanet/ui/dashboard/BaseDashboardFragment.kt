...
open class BaseDashboardFragment : BaseDashboardFragmentPlugin(), NotificationCallback,
    ...
    private lateinit var profileBinding: CardProfileBellBinding
    private lateinit var libraryBinding: HomeCardLibraryBinding
    private lateinit var libraryItemBinding: ItemLibraryHomeBinding
    private lateinit var courseBinding: HomeCardCoursesBinding
    private lateinit var meetupBinding: HomeCardMeetupsBinding
    private lateinit var teamBinding: HomeCardTeamsBinding
    private lateinit var healthListBinding: AlertHealthListBinding
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding = FragmentHomeBellBinding.bind(binding.root)
        profileBinding = CardProfileBellBinding.bind(profileBinding.root)
        libraryBinding = HomeCardLibraryBinding.bind(libraryBinding.root)
        libraryItemBinding = ItemLibraryHomeBinding.bind(libraryItemBinding.root)
        courseBinding = HomeCardCoursesBinding.bind(courseBinding.root)
        meetupBinding = HomeCardMeetupsBinding.bind(meetupBinding.root)
        teamBinding = HomeCardTeamsBinding.bind(teamBinding.root)
        onLoaded()
    }

    fun onLoaded() {
        // Update the references to views using the binding object.
        ...
        if (fullName?.trim().isNullOrBlank()) {
            ...
            binding.llPrompt.visibility = View.VISIBLE
            binding.llPrompt.setOnClickListener {
                UserInformationFragment.getInstance("").show(childFragmentManager, "")
            }
        } else {
            binding.llPrompt.visibility = View.GONE
        }
        ...
        profileBinding.txtVisits.text = """${profileDbHandler.offlineVisits} visits"""
        profileBinding.txtRole.text = """ - ${model.roleAsString}"""
        profileBinding.txtFullName.text = fullName
    }
    ...
    private fun myLibraryDiv() {
        libraryBinding.flexboxLayout.flexDirection = FlexDirection.ROW
        val dbMylibrary = RealmMyLibrary.getMyLibraryByUserId(mRealm, BaseResourceFragment.settings)
        if (dbMylibrary.size == 0) {
            libraryBinding.countLibrary.visibility = View.GONE
        } else {
            libraryBinding.countLibrary.text = dbMylibrary.size.toString()
        }
        var itemCnt = 0
        for (items in dbMylibrary) {
            ...
            ((v.findViewById<TextView>(R.id.title))).text = items.title
            v.findViewById<View>(R.id.detail).setOnClickListener {
                if (homeItemClickListener != null) homeItemClickListener.openLibraryDetailFragment(
                    items
                )
            }
            myLibraryItemClickAction(v.findViewById<TextView>(R.id.title), items)
            libraryBinding.flexboxLayout.addView(v, params)
            itemCnt++
        }
    }
    ...
    private fun setCountText(countText: Int, c: Class<*>, v: View) {
        if (c == RealmMyCourse::class.java) {
            updateCountText(countText, courseBinding.countCourse)
        } else if (c == RealmMeetup::class.java) {
            updateCountText(countText, meetupBinding.countMeetup)
        } else if (c == RealmMyTeam::class.java) {
            updateCountText(countText, teamBinding.countTeam)
        }
    }
    ...
    override fun showUserResourceDialog() {
        ...
        healthListBinding = AlertHealthListBinding.inflate(layoutInflater)
//            LayoutInflater.from(activity).inflate(R.layout.alert_health_list, null)
//        val btnAddMember = alertHealth.btn_add_member
        healthListBinding.etSearch.visibility = View.GONE
        healthListBinding.spnSort.visibility = View.GONE
        healthListBinding.btnAddMember.setOnClickListener {
            startActivity(
                Intent(
                    requireContext(),
...
//        sortList(spnSort, lv);
        dialog = AlertDialog.Builder(requireActivity()).setTitle(getString(R.string.select_member))
            .setView(healthListBinding.root).setCancelable(false).setNegativeButton("Dismiss", null)
            .create()
        dialog?.show()
    }
