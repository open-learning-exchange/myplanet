    ...
    inner class ViewHolderSurvey(private val binding: RowSurveyBinding) : RecyclerView.ViewHolder(binding.root) {
        ...
        fun bind(exam: RealmStepExam) {
            binding.apply {
                ...
                tvNoSubmissions.text = RealmSubmission.getNoOfSubmissionByUser(exam.id, exam.courseId, userId, mRealm)
                tvDateCompleted.text = RealmSubmission.getRecentSubmissionDate(exam.id, exam.courseId, userId, mRealm)
