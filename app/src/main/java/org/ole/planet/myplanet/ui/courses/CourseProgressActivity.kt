...
class CourseProgressActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            val uiState = loadCourseProgressData(courseId, user?.id)
            binding.progressView.setProgress(uiState.progress, true)
            binding.tvCourse.text = uiState.courseTitle
            binding.tvProgress.text = getString(
                R.string.course_progress,
                uiState.current,
                uiState.max
            )
            binding.rvProgress.adapter = AdapterProgressGrid(this@CourseProgressActivity, uiState.stepProgress)
        }
    }

    private suspend fun loadCourseProgressData(courseId: String, userId: String?): CourseProgressUiState = withContext(Dispatchers.IO) {
        databaseService.withRealm { realm ->
            val courseProgress = RealmCourseProgress.getCourseProgress(realm, userId)
            val progress = courseProgress[courseId]
            val courseObject = realm.where(RealmMyCourse::class.java).equalTo("courseId", courseId).findFirst()
            val course = if (courseObject != null) realm.copyFromRealm(courseObject) else null
            ...
            val current = progress?.get("current")?.asString ?: "0"
            val max = progress?.get("max")?.asString ?: "0"
