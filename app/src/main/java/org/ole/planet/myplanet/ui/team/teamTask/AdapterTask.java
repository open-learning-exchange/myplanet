...
public class AdapterTask extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof ViewHolderTask) {
            ...
            ((ViewHolderTask) holder).deadline.setText(context.getString(R.string.deadline_colon) + TimeUtils.formatDate(list.get(position).getDeadline()) + (list.get(position).isCompleted() ? "\n" +
                    context.getString(R.string.completed_colon) + TimeUtils.formatDate(list.get(position).getCompletedTime()) : ""));
    ...
    private void showAssignee(RecyclerView.ViewHolder holder, RealmTeamTask realmTeamTask) {
        if (!TextUtils.isEmpty(realmTeamTask.getAssignee())) {
            ...
            if (model != null) {
                ((ViewHolderTask) holder).assignee.setText(context.getString(R.string.assigned_to) + model.getName());
            }
        } else {
