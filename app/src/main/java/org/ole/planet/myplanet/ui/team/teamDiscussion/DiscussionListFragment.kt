...
class DiscussionListFragment : BaseTeamFragment() {
    ...
    private fun fetchNews() {
        lifecycleScope.launch {
            val results = newsRepository.getNewsByTeam(getEffectiveTeamId())
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            ...
            newsRepository.createNotification(getEffectiveTeamId(), count)
    ...
    private val news: List<RealmNews>
        get() {
            return runBlocking {
                val realmNewsList = newsRepository.getNewsByTeam(getEffectiveTeamId())
        ...
        if (existingAdapter == null) {
            val adapterNews = activity?.let {
                realmNewsList?.let { list ->
                    AdapterNews(it, list.toMutableList(), user, null, getEffectiveTeamName(), teamId, userProfileDbHandler, newsRepository, viewLifecycleOwner)
                }
            }
