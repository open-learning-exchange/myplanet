...
abstract class BaseRecyclerParentFragment<LI> : BaseResourceFragment() {
    ...
    suspend fun getList(c: Class<*>): List<LI> {
        return when {
            c == RealmStepExam::class.java -> {
                mRealm.where(c).equalTo("type", "surveys").findAll().let { mRealm.copyFromRealm(it) } as List<LI>
            }
            isMyCourseLib -> {
                getMyLibItems(c as Class<out RealmModel>)
            }
            c == RealmMyLibrary::class.java -> {
                val libs = resourcesRepository.getPublicLibraryItems()
                libs.filter { it.userId?.contains(model?.id) == false } as List<LI>
            }
            else -> {
                val myLibItems = getMyLibItems(c as Class<out RealmModel>)
                val realmResults = mRealm.where(RealmMyCourse::class.java)
                    .isNotEmpty("courseTitle")
                    .findAll()
                val results: List<RealmMyCourse> = mRealm.copyFromRealm(realmResults)
    ...
    suspend fun getList(c: Class<*>, orderBy: String? = null, sort: Sort = Sort.ASCENDING): List<LI> {
        return when {
            c == RealmStepExam::class.java -> {
                mRealm.where(c).equalTo("type", "surveys").sort(orderBy ?: "", sort).findAll().let { mRealm.copyFromRealm(it) } as List<LI>
            }
            isMyCourseLib -> {
                getMyLibItems(c as Class<out RealmModel>, orderBy)
            }
            c == RealmMyLibrary::class.java -> {
                val libs = resourcesRepository.getPublicLibraryItems() // Sorting ignored for now in repo
                libs.filter { it.userId?.contains(model?.id) == false } as List<LI>
            }
            else -> {
                val realmResults = mRealm.where(RealmMyCourse::class.java).sort(orderBy ?: "", sort).findAll()
                val results = mRealm.copyFromRealm(realmResults)
    ...
    private suspend fun <T : RealmModel> getMyLibItems(c: Class<T>, orderBy: String? = null): List<LI> {
        return when (c) {
            RealmMyLibrary::class.java -> {
                resourcesRepository.getLibraryByUserId(model?.id ?: "") as List<LI>
            }
            RealmMyCourse::class.java -> {
