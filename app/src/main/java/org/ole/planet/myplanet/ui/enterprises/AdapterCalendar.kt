...
class AdapterCalendar(private val list: List<RealmMeetup>) : RecyclerView.Adapter<ViewHolderCalendar>() {
    ...
    override fun onBindViewHolder(holder: ViewHolderCalendar, position: Int) {
        ...
        if (meetup.startDate == meetup.endDate) {
            ...
            rowTeamCalendarBinding.tvDate.text = formatDate(meetup.startDate) + " to " + formatDate(meetup.endDate)
        }
    }
