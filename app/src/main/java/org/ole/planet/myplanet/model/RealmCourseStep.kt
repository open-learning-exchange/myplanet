...
//    }
    ...
    companion object {
//        @JvmStatic
//        fun insertCourseSteps(myCoursesID: String?, steps: JsonArray, numberOfSteps: Int, mRealm: Realm, courstitle: String?) {
//            if (!mRealm.isInTransaction) {
//                for (step in 0 until numberOfSteps) {
//                    val stepContainer = steps[step].asJsonObject
//                    Log.d("ollonde", "title: $courstitle, number of steps: $numberOfSteps")
...
//                    for (step in 0 until numberOfSteps) {
//                        val stepContainer = steps[step].asJsonObject
//                        Log.d("ollonde", "execute title: $courstitle, number of steps: $numberOfSteps")
//
//                        val step_id = Base64.encodeToString(steps[step].toString().toByteArray(), Base64.NO_WRAP)
...
//                        }
//                        Utilities.openDownloadService(MainApplication.context, concatenatedLinks)
//                        myCourseStepDB?.noOfResources = JsonUtils.getJsonArray("resources", stepContainer).size()
//                        insertCourseStepsAttachments(myCoursesID, step_id, JsonUtils.getJsonArray("resources", stepContainer), realm)
...
//            val list: List<RealmMyCourse> = mRealm.where(RealmMyCourse::class.java).equalTo("courseId", courseId).findAll()
