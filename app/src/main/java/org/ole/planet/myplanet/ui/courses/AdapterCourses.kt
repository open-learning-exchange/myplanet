...
class AdapterCourses(
    ...
    companion object {
        ...
        val COURSE_ITEM_COMPARATOR = DiffUtils.itemCallback<CourseItem>(
    ...
    private fun configureDateViews(holder: ViewHoldercourse, course: CourseItem) {
        if (course.gradeLevel.isNullOrEmpty() && course.subjectLevel.isNullOrEmpty()) {
            ...
            try {
                holder.rowCourseBinding.tvDate2.text = formatDate(course.createdDate ?: 0L, "MMM dd, yyyy")
            } catch (e: Exception) {
            ...
            try {
                holder.rowCourseBinding.tvDate.text = formatDate(course.createdDate ?: 0L, "MMM dd, yyyy")
            } catch (e: Exception) {
                throw RuntimeException(e)
            }
        }
