...
class AdapterCourses(
...
) : ListAdapter<RealmMyCourse, RecyclerView.ViewHolder>(DIFF_CALLBACK) {
    ...
    override fun onBindViewHolder(
        ...
        if (hasTagPayload || hasRatingPayload || hasProgressPayload) {
            if (hasTagPayload) {
                val courseId = currentList.getOrNull(position)?.id ?: return
    ...
    private fun displayTagCloud(holder: ViewHoldercourse, position: Int) {
        ...
        val courseId = currentList.getOrNull(position)?.id
        if (courseId == null) {
    ...
    private fun updateRatingViews(holder: ViewHoldercourse, position: Int) {
        val course = currentList.getOrNull(position) ?: return
