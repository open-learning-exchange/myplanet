...
class ChallengeHelper(
    ...
    private data class ChallengeData(
        val uniqueDatesCount: Int,
        val allUniqueDatesCount: Int,
    ...
    fun evaluateChallengeDialog() {
        ...
        activity.lifecycleScope.launch {
            try {
                val challengeData = withContext(Dispatchers.IO) {
                    databaseService.withRealm { realm ->
                        ...
                        val course = realm.where(RealmMyCourse::class.java)
                            ...
                            .findFirst()
                        val unfinishedSurvey = hasPendingSurvey(realm, courseId)
                        ChallengeData(uniqueDates.size, allUniqueDates.size, course?.courseTitle, unfinishedSurvey)
                    }
                }
                ...
                val courseStatus = getCourseStatus(progress, challengeData.courseName)
                ...
                if (user?.id?.startsWith("guest") == false && shouldPromptChallenge(today, validUrls)) {
                    challengeDialog(challengeData.uniqueDatesCount, courseStatus, challengeData.allUniqueDatesCount, challengeData.hasUnfinishedSurvey)
                }
            } catch (e: Exception) {
                e.printStackTrace()
            }
    ...
    private suspend fun challengeDialog(voiceCount: Int, courseStatus: String, allVoiceCount: Int, hasUnfinishedSurvey: Boolean) {
        val hasValidSync = withContext(Dispatchers.IO) {
            databaseService.withRealm { realm ->
        ...
        val markdownContent = if (isCompleted && !hasShownCongrats) {
            ...
            val voicesText = if (cappedVoiceCount > 0) "$cappedVoiceCount ${activity.getString(R.string.daily_voices)}" else ""
            """
            ${activity.getString(R.string.community_earnings, viewModel.calculateCommunityProgress(allVoiceCount, hasUnfinishedSurvey))}
            ${activity.getString(R.string.your_earnings, viewModel.calculateIndividualProgress(voiceCount, hasUnfinishedSurvey))}
