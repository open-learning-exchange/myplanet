        ...
        private fun generateSubmissionPdf(submission: RealmSubmission) {
            ...
            Thread {
                val file = databaseService.withRealm { realm ->
                    val backgroundSubmission = realm.where(RealmSubmission::class.java)
                        .equalTo("id", submission.id)
                        .findFirst()
                    if (backgroundSubmission != null) {
                        SubmissionPdfGenerator.generateSubmissionPdf(context, backgroundSubmission, realm)
                    } else {
                        null
                    }
                }
