...
class TeamTaskFragment : BaseTeamFragment(), OnCompletedListener {
    ...
    private fun setAdapter() {
        if (isAdded) {
            ...
            adapterTask = AdapterTask(requireContext(), mRealm, list, !isMemberFlow.value, viewLifecycleOwner.lifecycleScope)
    ...
    private fun updatedTeamTaskList(updatedList: RealmResults<RealmTeamTask>) {
        viewLifecycleOwner.lifecycleScope.launch {
            adapterTask = AdapterTask(requireContext(), mRealm, updatedList, !isMemberFlow.value, viewLifecycleOwner.lifecycleScope)
