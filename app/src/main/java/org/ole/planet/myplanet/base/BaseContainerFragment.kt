...
abstract class BaseContainerFragment : BaseResourceFragment() {
    ...
    private fun checkFileExtension(items: RealmMyLibrary) {
        ...
        val existingAction = MainApplication.realm.query<RealmUserChallengeActions>(RealmUserChallengeActions::class, "userId == $0 AND resourceId == $1", userId, items.resourceId).first().find()
                ...
                when {
                    mimetype.contains("image") -> {
                        ...
                        if (existingAction == null) {
                            createAction(MainApplication.realm, userId, items.resourceId, "resourceOpen")
                        }
                    }
                    ...
                    else -> checkMoreFileExtensions(extension, items)
                }
            }
        }
    }
