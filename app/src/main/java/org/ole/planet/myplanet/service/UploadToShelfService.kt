    ...
    private suspend fun processUserAfterCreation(apiInterface: ApiInterface, model: RealmUserModel, obj: JsonObject, modelId: String) {
        try {
            ...
            if (fetchDataResponse.isSuccessful) {
                val body = fetchDataResponse.body()
                model.password_scheme = getString("password_scheme", body)
                model.derived_key = getString("derived_key", body)
    ...
    fun uploadHealth() {
        MainApplication.applicationScope.launch {
            ...
            val myHealths = dbService.withRealm { realm ->
                realm.where(RealmHealthExamination::class.java).equalTo("isUpdated", true).notEqualTo("userId", "").findAll().map { realm.copyFromRealm(it) }
            }
    ...
    fun uploadSingleUserHealth(userId: String?, listener: OnSuccessListener?) {
        MainApplication.applicationScope.launch {
            if (userId.isNullOrEmpty()) {
                withContext(Dispatchers.Main) {
                    listener?.onSuccess("Error: userId is empty")
                }
                return@launch
            }
