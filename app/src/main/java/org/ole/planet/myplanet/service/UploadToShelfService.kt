...
class UploadToShelfService(context: Context) {
        ...
        mRealm.executeTransactionAsync({ realm: Realm ->
            ...
            users.forEach { model ->
                try {
                    ...
                    apiInterface?.putDoc(Utilities.header, "application/json", "${Utilities.getUrl()}/shelf/${sharedPreferences.getString("userId", "")}", shelfObject)?.execute()?.body()
                } catch (e: Exception) {
                    e.printStackTrace()
                }
            }
    ...
    private fun getShelfData(realm: Realm?, userId: String?, jsonDoc: JsonObject?): JsonObject {
        ...
        val obj = JsonObject()
        obj.addProperty("_id", sharedPreferences.getString("userId", ""))
        obj.add("meetupIds", mergeJsonArray(myMeetups, getJsonArray("meetupIds", jsonDoc), removedResources))
        obj.add("resourceIds", mergedResourceIds)
        obj.add("courseIds", mergedCourseIds)
        return obj
    }
    ...
    fun syncShelf(listener: SuccessListener) {
        uploadToShelf(listener)
    }

    fun syncSingleUserShelf(userName: String?, listener: SuccessListener) {
