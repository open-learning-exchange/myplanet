...
private data class NewsItemData(
    val replyCount: Int,
...
class AdapterNews(
    ...
    private val newsDataCache = mutableMapOf<String, NewsItemData>()
    ...
    fun setData(newsList: List<RealmNews?>) {
        scope.launch(Dispatchers.IO) {
            val data = newsList.filterNotNull().associate { news ->
                news.id to NewsItemData(
                    replyCount = getReplies(news).size,
                    isTeamLeader = isTeamLeader()
                )
            }
            withContext(Dispatchers.Main) {
                newsDataCache.clear()
                newsDataCache.putAll(data as Map<String, NewsItemData>)
                submitList(newsList)
            }
        }
    }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder is ViewHolderNews) {
            ...
            if (news?.isValid == true) {
                ...
                val cachedData = newsDataCache[news.id]
                updateReplyCount(viewHolder, cachedData?.replyCount ?: 0, position)
                ...
                val canManageLabels = canAddLabel(news, newsDataCache[news.id]?.isTeamLeader ?: false)
    ...
    private fun showHideButtons(news: RealmNews?, holder: RecyclerView.ViewHolder) {
        ...
        val cachedData = news?.let { newsDataCache[it.id] }
        val isTeamLeader = cachedData?.isTeamLeader ?: false
        with(viewHolder.binding) {
    ...
    private fun showReplyButton(holder: RecyclerView.ViewHolder, finalNews: RealmNews?, position: Int) {
        ...
        val cachedData = finalNews?.let { newsDataCache[it.id] }
        updateReplyCount(viewHolder, cachedData?.replyCount ?: 0, position)
