    ...
    private fun renderTextFileThread() {
        lifecycleScope.launch(dispatcherProvider.io) {
            try {
                ...
                val oleDir = File(basePath, "ole")
                if (!file.canonicalPath.startsWith(oleDir.canonicalPath + File.separator)) {
                    throw SecurityException("Path traversal vulnerability: The file path is outside the intended directory.")
                }
