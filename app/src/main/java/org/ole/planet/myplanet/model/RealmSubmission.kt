    ...
    companion object {
        ...
        fun continueResultUpload(sub: RealmSubmission, apiInterface: ApiInterface?, realm: Realm, context: Context) {
            ...
            val `object`: JsonObject? = if (TextUtils.isEmpty(sub._id)) {
                apiInterface?.postDoc("application/json", UrlUtils.getUrl() + "/submissions", serializeExamResult(realm, sub, context))?.execute()?.body()
            } else {
                apiInterface?.putDoc("application/json", UrlUtils.getUrl() + "/submissions/" + sub._id, serializeExamResult(realm, sub, context))?.execute()?.body()
            }
            if (`object` != null) {
