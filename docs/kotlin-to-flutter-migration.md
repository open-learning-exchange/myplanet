...
- **The chat share (Phase 140) deviates in six places.** A shared conversation becomes a
  `news` document with `chat: true` and the transcript embedded under a nested `news`
  object, so the writer is `VoicesRepository.createFromShareMap` and the upload path is the
  voices one. Five of the six deviations are small; the first is the reason the list exists.
  ...
  - **`viewIn[].section` is written as the stable literal `community`/`teams`/`enterprises`.**
    ...
    `createTeamPost`), and nothing anywhere reads the value except that `"community"`
    comparison — in English the only observable difference is `R.string.enterprises`'
    capital E. This is the `9c037f2` lesson: routing on a localised display string is the
  ...
  - **A null conversation list is written as `[]`, not the string `"null"`.** Kotlin's
    ...
    running Gson 2.10 and 2.13.1, not by reading it, because the plausible guess is an
    uncaught `ClassCastException` and it is wrong). The inner `catch (e: JsonSyntaxException)`
    swallows it and the column is left null. The port writes `[]`, whose consumer also leaves
    the column null on an empty list. Same end state, no exception on the way.
  ...
  - **A chat the server has never seen reports `ChatShareOutcome.unavailable`; Kotlin drops
    ...
    payload's `newsId` is what every duplicate check keys on, so a chat with no `_id` has
    nothing to key on) and the port says so in a snackbar instead.
  ...
  - **A community row is not rendered when the planet has no community target.** Kotlin's
    ...
    flow is not a behaviour worth porting.
  ...
  - **The shared-destination map is recomputed when the dialog opens.** Kotlin's
    ...
    *unchanged* `sharedViewInIds` back to `adapter.updateCachedData`, so the checkmark does
    not appear until a realtime sync signal rebuilds the screen — and a second share of the
    same chat walks the whole dialog flow again before the snackbar says it was pointless.
  ...
  - **A successful share confirms itself with a snackbar, which Kotlin has no counterpart
    ...
    rebind, no message. So in Kotlin the *refused* share is the one that speaks and the
    successful one is silent.
