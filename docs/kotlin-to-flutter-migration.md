...
- **The chat share (Phase 140) deviates in six places, plus one line deliberately not
  carried across.** A shared conversation becomes a `news` document with `chat: true` and
  the transcript embedded under a nested `news` object, so the writer is
  `VoicesRepository.createFromShareMap` and the upload path is the voices one. Five of the
  six are small; the first is the reason the list exists.

  - **`viewIn[].section` is written as the stable literal `community`/`teams`/`enterprises`.**
    `ChatHistoryAdapter` passes the localised `strings.xml` value straight into
    `ChatSharePayload.buildShareMap` (`context.getString(R.string.teams)` and its two
    siblings), so a community share from a non-English handset writes a *translated* section
    and is invisible to `isVisibleToUser`, which matches `"community"`. The damage is in the
    stored document, so it is permanent and affects both apps. Every other writer on both
    sides already writes the literal (`VoicesFragment`, `TeamsVoicesFragment`, and the port's
    `createTeamPost`), and in English the only observable difference is
    `R.string.enterprises`' capital E, which nothing compares. This is the `9c037f2`
    lesson: routing on a localised display string is the bug, not the baseline.

    **Two things read that value, not one**, and the second is easy to miss because it is
    in another feature: besides `isVisibleToUser`, the challenge dialog's "post five
    community voices" tally counts only rows matching
    `viewIn LIKE '%"section":"community"%'` (`NewsDao.countDistinctCommunityVoiceDates`
    and `…ForUser`, reached through `VoicesRepositoryImpl.getCommunityVoiceDateCount`).
    So a Spanish-locale community share is *also* uncounted there — in both apps, since
    the port's `isCommunityNews` compares the same literal. Concretely, a share from a
    Spanish handset stores
    `viewIn: [{"_id":"gt@earth","section":"comunidad","name":null}]`, which is invisible
    in the community feed and invisible to the challenge.

  - **A null conversation list is written as `[]`, not the string `"null"`.** Kotlin's
    `chat.conversations?.map { … }` yields null, and `gson.toJson(null)` writes the four
    characters `null` into the nested object's `conversations` key. The reader then hands
    that to `fromJson(…, JsonArray::class.java)`, which raises `JsonSyntaxException`
    (*"Expected a com.google.gson.JsonArray but was com.google.gson.JsonNull"* — verified by
    running Gson 2.10, 2.13.1 and 2.14.0, the version `libs.versions.toml` pins, rather
    than by reading it, because the plausible guess is an uncaught `ClassCastException`
    and it is wrong). The exception *type* is load-bearing: a `ClassCastException` would
    escape both `catch (e: JsonSyntaxException)` blocks and crash out of
    `shareChatToVoices`' bare `viewModelScope.launch`. It does not — the inner catch
    swallows it, execution resumes, and the column is left null. The port writes `[]`,
    whose consumer also leaves the column null on an empty list.

    Two limits on "same end state". The **null**-list premise may be unreachable from
    Kotlin's own writers at all (`insertChatsBatchInternal` and `addConversation` both
    always assign a list); the reachable case is the *empty* list, which takes the
    no-exception path in Kotlin and is byte-identical to the port. And the end state is
    the same in the **local column only** — on the wire Kotlin's `serializeNews` emits
    `"conversations": null` where the port's `serialize` emits `[]`. Both mean "no
    transcript", and the documents differ.

  - **A chat the server has never seen reports `ChatShareOutcome.unavailable`; Kotlin drops
    the tap.** `ChatHistoryFragment`'s callback is
    `if (chatId.isNotEmpty() && viewInId.isNotEmpty()) { share }` with **no else**, so the
    user walks the target dialog, the picker and the note dialog, taps *share chat*, and
    nothing happens — no row, no upload, no snackbar, no log. The gate itself is kept (the
    payload's `newsId` is what every duplicate check keys on — a chat with no `_id` would
    store `newsId = ""` and collide with every other unsynced share) and the port says so
    in a snackbar instead. `ChatShareOutcome.unavailable` is reached from four conditions,
    not just this one: no signed-in user, an unsynced chat, no destination, and a
    rejecting `sessionProvider`.

  - **A community row is not rendered when the planet has no community target.** Kotlin's
    `shareDataMap` is a static `mapOf`, so the row renders and is tappable whether or not
    `shareTargets.community` resolved; picking it calls
    `showEditTextAndShareButton(null, …)`, the payload's `viewInId` is `""`, and the gate
    above discards the result. The port omits the row. Silence at the end of a four-step
    flow is not a behaviour worth porting. Note the asymmetry on the Kotlin side: the
    teams/enterprises branch *does* alert on an empty list, and only community does not.

  - **The shared-destination map is recomputed when the dialog opens.** Kotlin's
    `ShareChatResult.Shared` branch appends the new `News` to its list but passes the
    *unchanged* `sharedViewInIds` back to `adapter.updateCachedData` — the field is
    assigned in exactly one place, inside the `screenData` collector, and
    `shareChatToVoices` never re-runs it or emits `refreshChatSignal`. So the checkmark
    does not appear until something rebuilds the screen, and a second share of the same
    chat walks the whole dialog flow again before the snackbar says it was pointless. The
    branch is in fact **wholly inert**: the `sharedNewsMessages + result.news` it builds
    is stored as the adapter's `newsList`, which is written and never read, and
    `notifyChatShared` rebinds a row that holds no shared state. Two routes do refresh
    it — a realtime sync signal, and leaving the screen and coming back, because
    `_refreshChatSignal` is a `MutableSharedFlow(replay = 1)` that emits in `init`.
    `ChatShareActions.destinations()` recomputes on every open, which is what Kotlin does on
    every `refreshChatSignal`. Riverpod detail: it must be `refresh`, not `invalidate` then
    `read`, which still resolves the stale future in the same microtask. The "already
    shared" path stays reachable — another device, another user on the same planet, or a row
    whose `createdOn` differs from the viewer's planet code — and has its own test.

  - **`News.kt`'s `newsObj?.replace("=", ":")` is not ported, and dropping it preserves
    behaviour rather than diverging.** It is a fossil from when the value was a
    `HashMap.toString()` (`{_id=abc}`) rather than JSON, and it is a **no-op in Kotlin**
    for a reason invisible in the source: `JsonUtils.gson` is a bare `Gson()`, whose
    html-safe default already encodes `=` as `\u003d`, so the payload contains no literal
    `=` to replace. (Confirmed by running it, twice and independently — a title of `2+2=4`
    encodes as `{"title":"2+2\u003d4"}` and survives the round trip unchanged.) Dart's
    `jsonEncode` emits `=` raw, so porting the line would rewrite every `=` in the user's
    own title and transcript — `2+2=4`, a URL query string, base64 padding — and CouchDB
    would keep the damage. Listed here because the reasoning otherwise lives only at the
    Dart, and an omission with no note is the one a future reader is most likely to
    mistake for an oversight.

  - **A successful share confirms itself with a snackbar, which Kotlin has no counterpart
    for.** Kotlin snackbars only the `AlreadyShared` outcome; the `Shared` branch calls
    `notifyChatShared`, which is `notifyItemChanged(position, PAYLOAD_CHAT_SHARED)` — a row
    rebind of a row carrying no shared state, so the successful share has **no observable
...
- `efec5e7c6` (submissions repository exams starting) — centralizes
  ...
  **Closed in Phase 160**: both screens now share
  ...
  directions. `_surveyAnswer`'s carry-over stays — it guards a draft that is
  blank for any reason, not only one that arrived past that gate. The
...
> **Amended on harvest.** The session fix traded one silence for a narrower
> one. `await ref.read(sessionProvider.future)` sat *outside* the method's
> `try`, and awaiting a future means it can reject where
> `ref.read(...).valueOrNull` could not — so a rejecting session threw out of
> the async callback with no dialog and no snackbar, which is the exact
> failure mode the await was introduced to remove. Verified: on the pre-fix
> screen the new test gets `Exception: session unavailable` and finds zero
> snackbars. The await now has its own `catch`, reporting the same
> `examSubmitFailed` the save path does. 17 tests in the file.
