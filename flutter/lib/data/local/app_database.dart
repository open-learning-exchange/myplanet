...
class FeedbackDao extends DatabaseAccessor<AppDatabase>
  ...
  Stream<List<FeedbackRow>> watchAllSorted() =>
  ...
  Future<List<FeedbackRow>> getPending() =>
      (select(feedbackEntries)
            ..where((f) => f.isUploaded.equals(false)))
          .get();

  Future<FeedbackRow?> getById(String id) =>
      (select(feedbackEntries)..where((f) => f.id.equals(id))).getSingleOrNull();
  ...
  Future<void> updateRow(FeedbackEntriesCompanion row) =>
      (update(feedbackEntries)..where((f) => f.id.equals(row.id.value)))
          .write(row);
}
