...
class _FeedbackDetailScreenState extends ConsumerState<FeedbackDetailScreen> {
                  ...
                  child: SafeArea(
                    child: Row(
                      children: [
                        ...
                        if (isManager)
                          IconButton.filled(
                            onPressed: _isSubmitting ? null : () => _closeFeedback(feedback.id),
                        ...
                        IconButton.filled(
                          onPressed: _isSubmitting || session == null
                              ? null
                              : () => _submitReply(feedback.id, session.name ?? ''),
                          icon: _isSubmitting
  ...
  Future<void> _submitReply(String feedbackId, String userName) async {
    ...
    try {
      await ref.read(feedbackRepositoryProvider).addReply(feedbackId, message, userName);
